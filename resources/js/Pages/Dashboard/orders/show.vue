    ...
    <div class="mt-8">
      <div class="flex">
        <h2 class="text-3xl text-indigo-500 font-bold">
          الطلبات /<span class="text-gray-700"> توصيل الطلب</span>
        </h2>
      </div>
      <!-- md:max-w-3xl -->

      <base-panel class="mt-4">
        <p class="text-indigo-500 font-bold text-xl">
          رقم العميل :
          <span class="text-gray-700">{{ order.customer_phone }}</span>
        </p>
        <p class="text-indigo-500 font-bold text-xl">
          رقم العميل الأضافي :
          <span class="text-gray-700">{{ order.customer_alt_phone }}</span>
        </p>
        <p class="text-indigo-500 font-bold text-xl">
          عنوان العميل :
        ...
        <p class="text-indigo-500 font-bold text-xl">
          الخصم : <span class="text-gray-700">{{ order.discount }}</span>
        </p>
        <p class="text-indigo-500 font-bold text-xl">
          المبلغ الكلى :
        ...
        <p class="text-indigo-500 font-bold text-xl">
          التاريخ : <span class="text-gray-700">{{ order.created_at }}</span>
        </p>
        <p class="text-indigo-500 font-bold text-xl">
          الحالة :
          <span
            :class="[
              order.status == 'لم يتم التوصيل' ? 'text-red-600' : '',
        ...
        <div class="flex justify-start mt-4">
          <base-button danger @click="deleteOrder">حذف الطلب</base-button>
        </div>
      </base-panel>
      ...
      <base-panel class="mt-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div
            v-for="(orderDetail, key) in this.orderDetails"
            :key="key"
            ...
            >
              ...
              >
                <span class="text-gray-800 font-bold"
                  >SDG {{ orderDetail.product.total_price }}</span
                >
                <div>
                  <button
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              >
                ...
                <option value="تم الأعادة">تم الأعادة</option>
              </select>
              <span class="text-red-500 text-xs mt-4" v-if="$page.errors.status"
                >{{ $page.errors.status[0] }}
              </span>
            </div>
            <div>
              <label class="block">
              ...
              <span
                ...
                v-if="$page.errors.delivery_man_id"
                >{{ $page.errors.delivery_man_id[0] }}
              </span>
            </div>
            <div>
              <base-input
                ...
                :error="$page.errors.delivery_price"
                required
              >
              </base-input>
            </div>
            <div>
              <base-input
                ...
                option="أختياري"
                name="added_price"
                v-model="form.added_price"
                :error="$page.errors.added_price"
              >
              </base-input>
            </div>
            <div>
              <p class="text-gray-700">
                ...
                {{
                  +order.total_price +
                  (+this.form.delivery_price + +this.form.added_price)
                }}
              </p>
            </div>
