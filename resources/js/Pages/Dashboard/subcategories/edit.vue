    ...
    <div class="mt-8">
      <div class="flex">
        <h2 class="text-3xl text-indigo-500 font-bold">
          التصنيفات / التصنيفات الفرعية /<span class="text-gray-700">
            تعديل</span
          >
        </h2>
      </div>
      ...
      <base-panel class="mt-4">
        ...
        <div class="flex justify-start mt-4">
          <base-button danger @click="deleteSubcat"
            >حذف التصنيف الفرعي</base-button
          >
        </div>
      </base-panel>
    </div>
...
export default {
  ...
  methods: {
    submit() {
      this.$inertia.put(
        this.$route("subcategories.update", this.subcategory.id),
        this.form
      );
    },
    deleteSubcat() {
      this.$inertia.delete(
        this.$route("subcategories.destroy", this.subcategory.id)
      );
    },
  },
};
